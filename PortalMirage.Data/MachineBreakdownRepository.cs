...
public class MachineBreakdownRepository(IDbConnectionFactory connectionFactory) : IMachineBreakdownRepository
{
    ...
    public async Task<bool> MarkAsResolvedAsync(int breakdownId, int userId)
    {
        ...
        const string sql = """
                           ...
                           SET IsResolved = 1, ResolvedByUserID = @UserId, ResolvedDateTime = GETDATE() 
                           WHERE BreakdownID = @BreakdownId AND IsResolved = 0;
                           """;
        var rowsAffected = await connection.ExecuteAsync(sql, new { BreakdownId = breakdownId, UserId = userId });
